...
export function Settings(props: ISettingsProps) {
    ...
    const containerClases = ["settings__container", containerDark, cotainerActive]

    const rangeDark = settingsValue.isDark ? "settings__range-dark" : ""
    const rangeClases = ['settings__range', rangeDark]

    const openButtonState = isSettingsOpen ? 'settings__open-button-active' : 'settings__open-button-inactive'
    ...
    const openButtonClases = ["settings__open-button", openButtonState, openButtonStart, openButtonDark]

    const themeButtonDark = settingsValue.isDark ? 'settings__theme-button-dark' : ''
    const themeButtonClases = ["settings__theme-button", themeButtonDark]
        ...
        <div className={`settings`}>
            ...
            <div className={containerClases.join(' ')}>
                ...
                <div className="settings__block">
                    <label className="settings__label" htmlFor="workTime">Work Time</label>
                    <span className={rangeClases.join(' ')}>
                        <input type="range" name="workTime"
                            step={1}
                            defaultValue={settingsValue.workTime}
                            onChange={handleChange}
                            min={1} max={60} />
                    </span>
                    <span className="settings__curent-value">{`${settingsValue.workTime} min`}</span>
                </div>
                <div className="settings__block">
                    <label className="settings__label" htmlFor="smallBreakTime">Short Break Time</label>
                    <span className={rangeClases.join(' ')}>
                        <input type="range" name="smallBreakTime"
                            step={1}
                            defaultValue={settingsValue.smallBreakTime}
                            onChange={handleChange}
                            min={1} max={30} />
                    </span>
                    <span className="settings__curent-value">{`${settingsValue.smallBreakTime} min`}</span>
                </div>
                <div className="settings__block">
                    <label className="settings__label" htmlFor="bigBreakTime">Long Break Time</label>
                    <span className={rangeClases.join(' ')}>
                        <input type="range" name="bigBreakTime"
                            step={1}
                            defaultValue={settingsValue.bigBreakTime}
                            onChange={handleChange}
                            min={1} max={45} />
                    </span>
                    <span className="settings__curent-value">{`${settingsValue.bigBreakTime} min`}</span>
                </div>
                <div className="settings__block">
                    <label className="settings__label" htmlFor="bigBreakTime">Rounds</label>
                    <span className={rangeClases.join(' ')}>
                        <input type="range" name="amountOfBreaks"
                            step={1}
                            defaultValue={settingsValue.amountOfBreaks}
                            onChange={handleChange}
                            min={2} max={8} />
                    </span>
                    <span className="settings__curent-value">{`${settingsValue.amountOfBreaks}`}</span>
                </div>
                <div className="settings__block">
                    <SoundPicker settings={settingsValue} sendSettings={props.sendSettings}/>
                </div>
                <div className="settings__block">
                    <button className={themeButtonClases.join(' ')}
                        ...
                        <span>{`${settingsValue.isDark ? 'Dark Theme' : 'Light Theme'}`}</span></button>
                </div>
            </div>
        </div>
    )
